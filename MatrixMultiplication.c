...
void pmatrixTimesVectorNormalA_1(struct A_1_marked *A_1_marked,
    ...
    for (int i = 0; i < D; i++) {
        struct pol step_result;
        step_result = multiplyRowByVectorNormal(A_1_marked->pol[i],
                                                randomness->pol + D,
                                                K - D);
        step_result = addPolynomials(step_result,
                                     randomness->pol[i],
                                     get_N());
        commit->pol[i] = step_result;
    }
...
void pmatrixTimesVectorNormalA_2(struct A_2_marked *A_2_marked,
    ...
    for (int i = 0; i < L; i++) {
        struct pol step_result;
        step_result = multiplyRowByVectorNormal(A_2_marked->pol[i],
                                                randomness->pol + (D + L),
                                                K - D - L);
        step_result = addPolynomials(step_result,
                                     randomness->pol[i + D],
...
void pcommitNormal(struct A_1_marked *A_1_marked,
    ...
    for (int i = 0; i < D; i++) {
        commit->pol[i + L] = addPolynomials(commit->pol[i],
                                            message->pol[i],
                                            get_N());
    }
}
